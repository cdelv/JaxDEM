...
from jaxdem.minimizers.routines import MinimizeInfo, minimize
...
def lattice(dim=2):
    ...
    system = jd.System.create(
        state=state, collider_type="naive", dt=0.01,
        domain_type="periodic", domain_kw={"box_size": jnp.full(dim, 3.3)},
...
def test_outer_limit_preserves_last_evaluated_state(driver, outer_steps):
    ...
    result, info = driver(
        state, system, n_jamming_steps=outer_steps, n_minimization_steps=0,
        verbose=False, return_info=True,
    )
    assert isinstance(result, jamming.JamResult) and isinstance(info, jamming.JammingInfo)
    ...
    np.testing.assert_array_equal(result.jammed_system.domain.box_size, system.domain.box_size)
...
def test_three_dimensional_result_is_equilibrated(driver):
    ...
    kwargs = (
        dict(pe_tol=1e-6, pe_band_factor=2.0, packing_fraction_increment=0.03)
        if driver is jamming.pe_band_jam else
        dict(pressure_threshold=1e-4, pressure_band_factor=1.2, growth_rate=1.02,
             fine_growth_rate=1.002)
    )
    result, info = driver(state, system, n_minimization_steps=2, n_jamming_steps=80,
                          verbose=False, return_info=True, **kwargs)
    assert result.converged and info.converged and info.minimization.converged
    checked = minimize(result.jammed_state, result.jammed_system, max_steps=0, return_info=True)
    ...
    if driver is jamming.pe_band_jam:
        ...
        _, _, pressure = jamming.compute_contact_pressure(result.jammed_state, result.jammed_system)
...
def patched_minimize(monkeypatch):
    ...
    def install(energies, counts, statuses=None):
        ...
        statuses = jnp.zeros(len(counts), dtype=int) if statuses is None else jnp.asarray(statuses)

        def fake_minimize(state, system, **kwargs):
            index = state.vel[0, 0].astype(int)
            status = statuses[index]
            marker = jnp.full_like(state.vel, index + 1)
            info = MinimizeInfo(status == 0, status != 3, jnp.asarray(0.0),
                                jnp.asarray(0.0), status)
            return replace(state, vel=marker), system, counts[index], energies[index], info

        monkeypatch.setattr(jd.System, "minimize", staticmethod(fake_minimize))
        monkeypatch.setattr(
            jamming, "compute_contact_pressure",
...
def band_kwargs(driver):
    return (dict(pe_tol=1.0, pe_band_factor=2.0) if driver is jamming.pe_band_jam
            else dict(pressure_threshold=1.0, pressure_band_factor=2.0))


@pytest.mark.parametrize("driver", DRIVERS)
def test_maximum_includes_earlier_trials_and_returns_accepted_snapshot(driver, patched_minimize):
    patched_minimize([0.0, 0.0, 1.5], [2, 9, 1])
    state, system = lattice()
    result, info = driver(state, system, n_jamming_steps=3, n_minimization_steps=10,
                          verbose=False, return_info=True, **band_kwargs(driver))
...
def test_failed_trial_reports_nested_status_and_maximum(driver, min_status, patched_minimize):
    patched_minimize([0.0, 1.5], [2, 10], [0, min_status])
    state, system = lattice()
    result, info = driver(state, system, n_jamming_steps=4, n_minimization_steps=10,
                          verbose=False, return_info=True, **band_kwargs(driver))
    assert not result.converged and not info.converged
...
def test_pressure_initially_above_band_is_a_reported_failure(patched_minimize):
    ...
    result, info = jamming.pressure_bisection_jam(
        state, system, return_info=True, verbose=False, **band_kwargs(DRIVERS[1]),
    )
...
def test_pressure_bracket_exhaustion_is_a_reported_failure(patched_minimize):
    ...
    state, system = lattice()
    result, info = jamming.pressure_bisection_jam(
        state, system, return_info=True, verbose=False, length_ratio_tolerance=0.1,
...
def test_nonfinite_pressure_cannot_be_accepted(monkeypatch, patched_minimize):
    ...
    monkeypatch.setattr(jamming, "compute_contact_pressure", lambda st, sy, *args: (st, sy, jnp.nan))
    state, system = lattice()
    result, info = jamming.pressure_bisection_jam(state, system, return_info=True, verbose=False)
...
def test_pe_band_info_survives_jit_vmap():
    ...
    _, template = lattice()
    system = jd.System.create(state=state, collider_type="naive", dt=0.01,
                             domain_type="periodic", domain_kw={"box_size": jnp.full(2, 10.0)},
                             mat_table=template.mat_table, minimizer=jd.minimizers.fire)

    def run(separation):
        trial = replace(state, pos_c=state.pos_c.at[1, 0].set(separation))
        return jamming.pe_band_jam(trial, system, n_minimization_steps=0,
                                   n_jamming_steps=1, verbose=False, return_info=True)
