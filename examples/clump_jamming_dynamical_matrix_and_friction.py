...
Instead of point-like spheres, we build a 2D packing of rigid clumps
with mixed sizes and shapes (a few different vertex-counts and bounding
radii). We jam it and analyze the dynamical matrix in the full
rigid-body coordinate space :math:`(\delta r, \omega)` per clump.

A rigid body in 2D has :math:`d_f = \dim + \dim_{\rm rot} = 3` degrees
of freedom (:math:`d_f = 6` in 3D). A rattler with ``k``
...
:math:`d_f` generic contacts gives no floppy modes. The default
...
#
# We use a bidisperse mix of shapes and sizes for the same reason as the
# sphere example. It suppresses the crystalline packings that a
# monodisperse system would fall into. The zero-mode spectrum then stays
...
# :func:`~jaxdem.utils.contacts.get_clump_rattler_ids` also uses the
...
# generalized coordinates :math:`(\delta r_c, \omega)`. Each clump has a
...
# :func:`~jaxdem.utils.contacts.remove_rattlers`. It drops their
...
# and they can give :math:`\mu > 0`. This is the purely geometric
