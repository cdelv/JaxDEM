...
if __name__ == "__main__":
    ...
    for n in n_values:
        print(f"\nTesting N={n}")
        for c in colliders:
            throughput = run_benchmark(n, dim, c, n_steps=1000, neighbor_list=True)
    ...
    plt.title(f"Collider Neighbor List Throughput Benchmark ({dim}D Spheres)")
    ...
    plt.yscale("log")
    plt.tight_layout()
    plt.savefig(f"benchmark_neighbor_list_results_{dim}D.png")
    print(f"\\nBenchmark plot saved to benchmark_neighbor_list_results_{dim}D.png")
