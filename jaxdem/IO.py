...
class VTKBaseWriter(Factory, ABC):
    @classmethod
    @abstractmethod
    def write(cls, state: 'State', system: 'System', save_counter: int, data_dir: str, binary: bool) -> int:
        """
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers = len(self.writers)) as executor:
            for i in range(batch_size):
                ...
                state_i = jax.tree_map(
                    lambda x: x[i] if (isinstance(x, jax.Array) and x.shape[0] == batch_size) else x,
                    state
                )
