...
def _sample_objectives(key: ArrayLike, N: int, box: jax.Array, rad: float) -> jax.Array:
    ...
    if N == 0:
        return jnp.zeros((0, 2), dtype=box.dtype)
...
class SwarmNavigator(Environment):
    r"""Multi-agent navigation environment toward nearby shared targets.
    ...
    step. Objectives are sampled globally, and each agent observes objective
    LiDAR and agent LiDAR.
    ...
    def reset(env: "SwarmNavigator", key: ArrayLike) -> Environment:
        ...
        pos_mid = _sample_objectives(key_pos_mid, n_middle, box, rad)
        ...
        left_box = jnp.asarray([strip_w, full_h])
        right_box = jnp.asarray([strip_w, full_h])
        bottom_box = jnp.asarray([box[0], strip_w])
        top_box = jnp.asarray([box[0], strip_w])

        left_anchor = jnp.asarray([-strip_w, -strip_w])
        right_anchor = jnp.asarray([box[0], -strip_w])
        bottom_anchor = jnp.asarray([0.0, -strip_w])
        top_anchor = jnp.asarray([0.0, box[1]])

        pos_left = _sample_objectives(key_left, n_left, left_box, rad) + left_anchor
        pos_right = (
            _sample_objectives(key_right, n_right, right_box, rad) + right_anchor
        )
        pos_bottom = (
            _sample_objectives(key_bottom, n_bottom, bottom_box, rad) + bottom_anchor
        )
        pos_top = _sample_objectives(key_top, n_top, top_box, rad) + top_anchor
        ...
        deltas = env.system.domain.displacement(
            env.state.pos[:, None, :],
            env.env_params["objective"][None, :, :],
            env.system,
        )
        dist_all = norm(deltas)
        closest_dist = jnp.min(dist_all, axis=1)
        env.env_params["curr_dist"] = closest_dist
    ...
    def observation_space_size(self) -> int:
        ...
        dim = self.state.dim
        return dim + 2 * self.n_lidar_rays
